...
The main dmaorg website started off with five images - over time, more images are added with each passing update. Each image has a unique file name and features a MOON date above it, which is a date shown in a <YYY MM(MOON) DD> format on the site - the YYY corresponds to the last three digits of the year. Most MOON dates correlate to either a relevant date in the band's history or, in later updates, the day of the current update.
