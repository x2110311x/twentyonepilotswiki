...
The main dmaorg website started off with five images on its page. Over time, more images were added with each passing update. Each image has a unique file name and features a MOON date above it, which is a date shown in a <YYY MM(MOON) DD> format on the site - the YYY corresponds to the last three digits of the year. Most MOON dates correlate to either a relevant date in the band's history or, in later updates, the day of the current update.
...
- The metadata of the file reveals a hidden MOON date: cla_ncy-009-06MOON_14_-1 (June 14th, 2009). Although it is not confirmed, the date is suspected to be the founding of the band, as it seems to be around the time when they purchased the ‘band house’ where their first album was recorded.
...
- This picture is a half-toned, cropped version of a photo (taken on December 31st, 1955) featuring Andre Weil on the left and a child, whose identity is yet unknown, covering their face. Andre Weil was a famous mathematician who introduced the use of “∅” to denote the empty set (used on the TØP official logo). He was also a member of a group of mathematicians known as the Bourbaki group, which collectively wrote under the pseudonym Nicolas Bourbaki.
...
- The missing corner of the map contained a compass hidden through transparency that could only be revealed by digitally recoloring the blank space of the the original image that was posted. The map currently displayed on the site does not feature this corner anymore.
...
The text under the caption of the map says:
